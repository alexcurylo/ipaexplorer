...
- (CTGradientElement *)elementAtIndex:(unsigned)theIndex;

- (CTGradientElement)removeElementAtIndex:(unsigned)theIndex;
...
{
   CTGradient *newGradient = [self copy];
   CTGradientElement removedElement = [newGradient removeElementAtIndex:theIndex];
   
   if(isnan(removedElement.position))
      [NSException raise:NSRangeException format:@"-[%@ removeColorStopAtIndex:]: theIndex (%i) beyond bounds", [self class], theIndex];
...
{
   CTGradientElement *element = [self elementAtIndex:theIndex];
