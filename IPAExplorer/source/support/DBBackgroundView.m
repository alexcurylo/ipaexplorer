...
{	
	...
	if (_backgroundStrokeColor)
		[_backgroundStrokeColor release], _backgroundStrokeColor = nil;
	...
	if (_3SliceLeftImage)
		[_3SliceLeftImage release], _3SliceLeftImage = nil;
	if (_3SliceMiddleImage)
		[_3SliceMiddleImage release], _3SliceMiddleImage = nil;
	if (_3SliceRightImage)
		[_3SliceRightImage release], _3SliceRightImage = nil;
...
- (void)setBackgroundStrokeColor:(NSColor *)aColor
{
	if (_backgroundStrokeColor)
		[_backgroundStrokeColor release], _backgroundStrokeColor = nil;
   
   if (aColor)
      _backgroundStrokeColor = [aColor retain];
}
...
- (void)set3SliceBackgroundImages:(NSImage *)left middle:(NSImage *)middle right:(NSImage *)right
{
   [left retain];
   [middle retain];
   [right retain];
   [_3SliceLeftImage release];
   [_3SliceMiddleImage release];
   [_3SliceRightImage release];

   _3SliceLeftImage = left;
   _3SliceMiddleImage = middle;
   _3SliceRightImage = right;
}
...
- (void)drawColor:(NSBezierPath *)aPath
{
	if (_backgroundColor)
   {
		NSColor *fillColor = [_backgroundColor colorWithAlphaComponent:_backgroundAlpha];
		[fillColor set];
		[aPath fill];	
	}
	...
	if (_backgroundStrokeColor)
   {
		NSColor *strokeColor = [_backgroundStrokeColor colorWithAlphaComponent:_backgroundAlpha];
		[strokeColor set];
...
- (void)draw3SliceImages:(NSBezierPath *)path
{
   // for now we'll assume this would be an opaque view-filling background set
   (void)path;
   
   if (!_3SliceLeftImage || !_3SliceMiddleImage || !_3SliceRightImage)
      return;
   
   NSPoint leftOrigin = [self bounds].origin;
   //leftOrigin.y = 0; // bound to bottom 
   leftOrigin.y = NSMaxY([self bounds]) - [_3SliceLeftImage size].height; // bound to top
	[_3SliceLeftImage compositeToPoint:leftOrigin operation:NSCompositeCopy];
   
   NSPoint rightOrigin = NSMakePoint(
      NSMaxX([self bounds]) - [_3SliceRightImage size].width, 
      // 0 // bound to bottom
      NSMaxY([self bounds]) - [_3SliceLeftImage size].height // bound to top
   );
	[_3SliceRightImage compositeToPoint:rightOrigin operation:NSCompositeCopy];

   NSRect middleRect = [self bounds];
   middleRect.origin.x += [_3SliceLeftImage size].width;
   middleRect.size.width -= [_3SliceLeftImage size].width;
   middleRect.size.width -= [_3SliceRightImage size].width;
   //middleRect.origin.y = 0; // bound to bottom
   middleRect.origin.y = NSMaxY([self bounds]) - [_3SliceMiddleImage size].height; // bound to top
   middleRect.size.height = [_3SliceMiddleImage size].height;
	[_3SliceMiddleImage drawInRect:middleRect fromRect:NSZeroRect operation:NSCompositeCopy fraction:1.];
}
